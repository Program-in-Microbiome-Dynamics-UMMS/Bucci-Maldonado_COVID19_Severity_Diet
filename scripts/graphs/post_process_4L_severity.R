...
##### load data #####
#fileName_1 <- "../../results/RF_final_Sev_aggregate_taxonomy_object_oriented/2021-03-28/ml_classification_severity_REDEMSVIR_SEVERITY_batch_relab.Rdata"
#fileName_1 <- "../../results/RF_final_Sev_aggregate_taxonomy_object_oriented/2022-07-08/ml_classification_severity_REDEMSVIR_SEVERITY_batch_vst.Rdata"
#fileName_1 <- "../../results/RF_final_Sev_aggregate_taxonomy_object_oriented_keep_antibiotics//2022-07-08/ml_classification_severity_REDEMSVIR_SEVERITY_batch_vst.Rdata"
#fileName_1 <- "../../results/RF_final_severity_4L_aggregate_taxonomy_object_oriented_relab_F/2022-07-10/ml_classification_severity_REDEMSVIR_SEVERITY_batch_vst.Rdata"
#fileName_1 <- "../../results/RF_final_severity_4L_aggregate_taxonomy_object_oriented_relab_F/2022-07-11/ml_classification_severity_REDEMSVIR_SEVERITY_batch_vst.Rdata"
#fileName_1 <- "../../results/RF_final_severity_4L_aggregate_taxonomy_object_oriented_relab_T/2022-07-12/ml_classification_severity_REDEMSVIR_SEVERITY_batch_relab.Rdata"
#fileName_1 <- "../../results/RF_final_severity_4L_aggregate_taxonomy_object_oriented_T_includingantibiotics/2022-07-16/ml_classification_severity_REDEMSVIR_SEVERITY_batch_relab.Rdata"
fileName_1 <- "../../results/SEVERITY-RF_final_severity_4L_aggregate_taxonomy_object_oriented_relab_F/SEVERITY-2022-07-11/ml_classification_severity_REDEMSVIR_SEVERITY_batch_vst.Rdata"
...
ggsave(paste0(fig_folder, "/Importance_",analysis_name,".pdf"), g_importance, width=20, height =22)
...
res_importance_summary_temp <- res_importance_summary[res_importance_summary$data_name=="CC_STL_ASV",]
features_to_plot <- res_importance_summary_temp$var_name
features_to_plot
features_to_plot_micro <- features_to_plot[grep("mic_", features_to_plot)]
xdata <- data_list["CC_STL_ASV"]
xdata<- xdata$CC_STL_ASV[,c("class", features_to_plot_micro)]
x_data_m <- melt(xdata)
...
violing_microbiome <- ggplot(x_data_m, aes(class, value, fill = class))+
  ...
  theme_prism()
ggsave(paste0(fig_folder, "/Abundance_microbiome_","CC_STL_ASV",analysis_name,".pdf"), violing_microbiome, width=22, height =12)


# SIRUS analysis
library(sirus)
res_importance <- do.call('rbind',res_importance_list)
res_importance_sub <- res_importance[res_importance$data_name=="CC_STL_ASV",]
boruta_sp <- unique(res_importance_sub$var_name)
t_data <- data_list$CC_STL_ASV
...
ir <- 1
pdf(paste0(fig_folder, "/sirus_results","CC_STL_ASV",".pdf"), width=8, height =8)
for (ir in seq(1,nrow(all_rule_dt_microbiome))){
  ...
  if (isalone == F){
    ...
    xdata <- data_list["CC_STL_ASV"]
    xdata<- xdata$CC_STL_ASV[,c("class", all_rule_dt_microbiome$Taxa[ir])]
    x_data_m <- melt(xdata)
...
write.csv(all_rule_dt,"./sirus.csv")
